...
- **Margin math, why there's no sub-RM15 package**: Billplz charges a **flat** per-FPX-transaction fee (~RM0.75–1.25 depending on account tier), not a percentage — so it hits small purchases hardest. A since-dropped 100-token/RM5 "Starter" tier netted only RM0.25–0.75 margin (5–15%) after the Billplz fee (RM0.75–1.25) and full Claude cost if all 100 tokens got used (100 × ~RM0.035 ≈ RM3.50) — fragile enough that a slightly-larger-than-estimated image or a USD/MYR move could flip it negative. The kept tiers fare much better: Standard (300 tok/RM15) nets ~RM3.25–3.75 margin (~22–25%), Bulk (1000 tok/RM50) nets ~RM13.75–14.25 (~28–29%) — the flat Billplz fee amortizes better over a bigger purchase. **How to apply**: if a new package tier is ever added, redo this same three-way check (revenue − flat payment fee − full Claude cost of the tokens) before shipping it, especially for anything below ~RM15.
