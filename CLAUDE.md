...
**Mobile canvas overflow bug, fixed 2026-08-22**: `bbox-web`'s `BBoxCanvas.tsx` used to set the `<canvas>` element's CSS `width`/`height` directly to its internal drawing resolution (up to `MAX_WIDTH = 800`px), with no regard for viewport width. On a phone (~375–430px CSS width), the canvas overflowed its container — the page scrolled horizontally instead of the image scaling down, and since the canvas also sets `touch-action: none`, touching it didn't pan the page either, so users on `bboxai-remote` could only see/reach the initial slice of the image and couldn't draw boxes on the rest. Confirmed **not** an iOS/Safari-specific issue — `index.html` already has `<meta name="viewport" content="width=device-width, initial-scale=1.0">`, so Safari uses the real device width same as any mobile browser; the bug and fix are identical across browsers. **Fix**: canvas now renders at `width: 100%; height: auto` of its wrap (wrap changed from `display: inline-block` to `width: 100%`, capped via inline `maxWidth: size.width`) — the canvas element's `width`/`height` *attributes* (drawing resolution) are untouched, only the CSS display size scales down, same as an `<img>` with `height: auto`. `getPos()`'s pointer-coordinate math already normalizes through `getBoundingClientRect()`, so it was already display-size-agnostic and needed no changes. Deployed live to `bboxai-remote.unitani.com` (commit `119b1a2`).

**Follow-up, same day: resize handles/box lines/class-label too thin to use on mobile.** Once the canvas itself scaled correctly, its *contents* were still a problem — handle boxes, stroke widths, and the class-name label font were drawn as small fixed sizes in canvas-internal-pixel space (e.g. an 8×8px handle, 2–3px lines). On desktop the canvas displays at ~1:1 with its internal resolution so this looked fine, but on mobile the canvas is now CSS-downscaled (e.g. 800px internal → ~380px displayed), which shrinks those fixed sizes to a barely-visible/hard-to-grab ~half their size on screen. **Fix**: `BBoxCanvas.tsx` now computes `scale = size.width / canvas.getBoundingClientRect().width` in `draw()` (stored in `scaleRef` so pointer-event hit-testing outside `draw()` can read it too) and multiplies every on-screen-sized constant — handle box (`HANDLE_SIZE_CSS` 14px), handle hit-test tolerance (`HANDLE_HIT_CSS` 22px, intentionally bigger than the visible handle for touch), box stroke width (`LINE_WIDTH_CSS` 2.5 / `LINE_WIDTH_SELECTED_CSS` 4), and the class-name label's font size + badge height — by it before drawing/hit-testing. Keeps all of these a consistent real-world (CSS pixel) size regardless of how much the canvas is downscaled, on any device.
...
- **Validation loop (2026-08-19)**: `bbox-web`'s `AnnotatePage.tsx` tags each AI-suggested box with a client-only `_aiId` at suggestion time and keeps a snapshot of its original `{x,y,w,h,class_id}` (`aiSnapshotRef`, keyed by `_aiId`) separate from the live `boxes` state — needed because a deleted box just disappears from `boxes`, so only the snapshot still knows it existed. On commit, each snapshot id is classified against the final saved boxes: **accepted** (still present, unchanged within a small epsilon), **edited** (still present, coordinates/class differ), or **deleted** (id no longer present) — then posted as aggregate counts (not per-box, no coordinates) to `bbox-relay`'s `POST /ai-assist/feedback`, best-effort and non-blocking. `GET /ai-assist/feedback/summary` (optionally filtered by `project_id`) returns the caller's aggregate accept rate. This is deliberately the coarse half of a two-loop design the user proposed — validation was built first since it's useful standalone (an accuracy trend metric) even without changing how examples are selected.

- **Correction loop, capture side (2026-08-21)**: `models.AiAssistCorrection` (`bbox-relay`) stores the real per-box before/after diff behind the aggregate counts above — `POST /ai-assist/corrections` gets one entry per suggested box (`outcome` + `original`/`final` coords), sent alongside the `/ai-assist/feedback` call in `reportAiAssistValidation`. Nothing read this table until the consumption side below.
- **Correction loop, consumption side (2026-08-22)**: `bbox-relay`'s `example_ranking.py` scores each project image that's ever been used as a few-shot example by the outcomes of AI-assist calls that included it — `accepted` = 1, `edited` = 0.5, `deleted` = 0, Bayesian-smoothed toward a neutral prior (`_PRIOR_MEAN = 0.75`, `_PRIOR_COUNT = 3`) so one unlucky call or an unused image doesn't swing the score. **Attribution limitation, deliberate**: a single call sends 2–5 examples together and a correction is only known per suggested box on the *target* frame, so there's no way to isolate which one example caused a given box to be wrong — every example present in a call's set is credited/blamed equally for that call's outcomes; this separates consistently-good from consistently-poor examples only in aggregate, over many calls, not per-call. `AiAssistCorrection` gained a nullable `example_image_ids` column (JSON list, denormalized onto each row from the same commit) to make this possible — old rows from before this column existed are just skipped by the scoring query. New `GET /ai-assist/example-ranking?project_id=` returns `{image_id: {score, n}}` for the caller. `bbox-web`'s `gatherExamples()` (`AnnotatePage.tsx`) now sorts candidate example images by this score (falling back to `DEFAULT_EXAMPLE_SCORE = 0.75`, matching the relay's prior, for unscored images) before taking the top `MAX_AI_ASSIST_EXAMPLES`, instead of always taking the first N by upload order — deprioritizes proven-poor examples without ever excluding an unproven one, and degrades gracefully to the old upload-order behavior if the ranking fetch fails. **How to apply**: if a real per-example (not per-call) attribution signal is ever wanted, this whole equal-credit approach would need replacing (e.g. leave-one-out experiments varying which examples are sent) — not a small tweak to `example_ranking.py`.
