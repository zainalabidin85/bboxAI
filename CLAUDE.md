...
- **`bbox-api` itself never changes behavior for self-hosted/desktop users**: its own `GET /projects/{id}/report` gained a `tier` query param defaulting to `"paid"` — a local/desktop caller that never passes `tier` keeps getting the exact same full report, free, as before this feature existed. Only `bbox-relay`'s report-unlock flow ever explicitly requests `tier=free`.
